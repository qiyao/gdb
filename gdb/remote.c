...
static void notif_queued_reply_push (struct notif_queued *notif,
...
extended_remote_attach_1 (struct target_ops *target, char *args, int from_tty)
{
  ...
  if (!non_stop)
      ...
      if (target_can_async_p ())
	{
	  ...
	  notif_queued_reply_push (&notif_packet_stop, reply);
...
remote_notif_ack_stop (struct notif *self, char *buf, void *data)
{
  ...
  notif_queued_reply_push ((struct notif_queued *) self,
...
struct notif_queued notif_packet_stop =
{
  {
    "Stop",
    "vStopped",
...
static void
notif_queued_reply_push (struct notif_queued *notif,
			 struct notif_reply *new_event)
{
  gdb_assert (notif == &notif_packet_stop);
